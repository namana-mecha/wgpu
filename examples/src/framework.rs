...
use wgpu::{rwh::HasDisplayHandle, Backends, Instance, Surface};
use winit::{
    dpi::PhysicalSize,
...
impl ExampleContext {
    ...
    async fn init_async<E: Example>(surface: &mut SurfaceWrapper, window: Arc<Window>) -> Self {
        ...
        instance_desc.display = Some(window.display_handle().unwrap().as_raw());
        instance_desc.backends = Backends::GLUTIN;
